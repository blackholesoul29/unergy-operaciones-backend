"""Initial database schema setup (baseline).

Revision ID: 000_baseline
Revises:
Create Date: 2026-06-20

Single baseline migration that materializes the COMPLETE current schema.

Context
-------
This project previously managed its schema by running idempotent DDL on every
application boot (``app/main.py``) and inside ``init_db.py`` via
``Base.metadata.create_all`` + hand-written ``ALTER TABLE`` statements. The
pre-existing ``alembic/versions`` chain was non-functional (duplicate revision
ids ``020``/``024`` and no base-table-creating migration), so Alembic was never
actually the source of truth. Those legacy scripts are archived under
``alembic/versions/legacy_pre_baseline/`` for historical reference.

This baseline freezes the exact bootstrap behaviour the application performed at
runtime into a single Alembic revision, so that from now on Alembic is the sole
mechanism for schema creation and evolution.

``upgrade()`` reproduces the historical bootstrap faithfully:

1. ``Base.metadata.create_all`` — creates every ORM-mapped table (69 tables) and
   the PostgreSQL ENUM types backing ``Enum`` columns.
2. The idempotent DDL block below (relocated verbatim from ``app/main.py``) —
   creates the ~17 raw-SQL-only tables that have no ORM model
   (``audit_log``, ``api_keys``, ``clima_*``, ``om_*``, ``precios_bolsa_*``,
   ``informes_guardados``, ``alarma_estado``, ``alarmas_monitoreo``, ...) plus
   any belt-and-suspenders columns/indexes/enum values.

Every statement is ``IF [NOT] EXISTS`` / try-excepted, so running this baseline
against a database that already has the schema is a safe no-op. For an existing
production database the recommended one-time step is instead
``alembic stamp 000_baseline`` (see docs/MIGRATIONS.md).
"""
...
# ---------------------------------------------------------------------------
# Idempotent DDL — relocated verbatim from app/main.py (_PENDING_DDLS).
# Covers raw-SQL tables/columns/enums that are NOT defined as ORM models.
# ---------------------------------------------------------------------------
_PENDING_DDLS = [
...
def _run_pending_ddls(engine) -> None:
    """Run the idempotent DDL exactly as app/main.py did at startup.

    ``ALTER TYPE ... ADD VALUE`` cannot run inside a transaction block in
    PostgreSQL, so those run first on an AUTOCOMMIT connection; the remaining
    statements run with per-statement commit/rollback so one failure never
    aborts the rest.
    """
    ...
    for stmt in add_value_stmts:
        try:
            ...
            print(f"[baseline ddl skipped] {e}")
...
# Raw-SQL-only tables (no ORM model) created by _PENDING_DDLS, dropped on downgrade.
_NON_MODEL_TABLES = [
    ...
    "informes_guardados", "om_factura_mensual", "om_ipc_tasas",
    "om_seleccion_mensual", "precios_bolsa_diario", "precios_bolsa_horario",
]


def upgrade() -> None:
    # Self-contained bootstrap on the app engine. We deliberately use the
    # application engine (not op.get_bind()) so the AUTOCOMMIT handling for
    # ``ALTER TYPE ... ADD VALUE`` matches the historical behaviour and never
    # collides with Alembic's own migration transaction.
    from app.core.database import engine
    from app.models import Base

    Base.metadata.create_all(bind=engine)
    _run_pending_ddls(engine)
...
def downgrade() -> None:
    ...
    with engine.connect() as conn:
        for tbl in _NON_MODEL_TABLES:
            try:
                conn.execute(text(f"DROP TABLE IF EXISTS {tbl} CASCADE"))
                conn.commit()
            except Exception as e:  # noqa: BLE001
                conn.rollback()
                print(f"[baseline downgrade skipped] {e}")

"""Consolidar DDL idempotente migrado desde _PENDING_DDLS (app/main.py)

DDL idempotente trasladado verbatim desde la lista `_PENDING_DDLS` de
app/main.py, que históricamente se ejecutaba en cada arranque de la app
(`_run_column_migrations`). Se mueve a Alembic para que las migraciones sean
la única fuente de verdad de la evolución del esquema y la app no modifique el
esquema en tiempo de ejecución. Las sentencias se conservan sin cambios.

Revision ID: 026_pending_ddls_main
Revises: 025
...
revision = "026_pending_ddls_main"
down_revision = "025"
branch_labels = None
depends_on = None

REV = "026_pending_ddls_main"

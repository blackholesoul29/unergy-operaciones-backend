"""047 -- consolidate startup DDL safety-net from app/main.py

Historically ``app/main.py`` applied a large list of idempotent DDL statements on
every boot -- the module-level ``_PENDING_DDLS`` list, applied by
``_run_column_migrations`` from the deferred-init startup hook. That made the
running application, rather than Alembic, the de-facto source of truth for the
schema. This migration version-controls that exact DDL so Alembic is the single,
auditable source of truth.

Every statement is idempotent (``IF NOT EXISTS`` / guarded ``UPDATE`` /
``ADD VALUE IF NOT EXISTS`` / bare ``CREATE TYPE`` that is skipped when the type
...
# Verbatim copy of the former ``app/main.py`` ``_PENDING_DDLS`` list.
_STATEMENTS = [

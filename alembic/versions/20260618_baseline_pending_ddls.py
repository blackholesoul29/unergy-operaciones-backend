"""baseline: idempotent DDL previamente ejecutado en cada arranque

Esta migracion consolida, dentro del control de versiones de Alembic, todo el
DDL idempotente que hasta ahora se ejecutaba de forma imperativa en cada boot:

  * ``_PENDING_DDLS`` de ``app/main.py`` (corrido por ``_run_column_migrations``
    dentro del arranque diferido), y
  * los ``ALTER TABLE`` / ``ALTER TYPE`` de ``add_columns()`` en ``init_db.py``.

Ambos mecanismos se eliminaron; el esquema ahora es responsabilidad de Alembic.
Los ``CREATE ... IF NOT EXISTS`` / ``ADD COLUMN IF NOT EXISTS`` / ``ADD VALUE IF
NOT EXISTS`` son idempotentes, por lo que aplicar esta migracion sobre una base
que YA los tiene (produccion) es un no-op seguro. Los ``ADD VALUE`` de enum se
corren en AUTOCOMMIT (PostgreSQL no permite ``ALTER TYPE ... ADD VALUE`` dentro
de un bloque transaccional); el resto se corre con tolerancia por-sentencia (el
mismo comportamiento que tenia ``_run_column_migrations``) para que una sentencia
no-idempotente ya aplicada (p. ej. ``ADD CONSTRAINT``) no aborte la migracion.
...
def _run(stmts) -> None:
    """Ejecuta las sentencias replicando la tolerancia de ``_run_column_migrations``.

    - ``ALTER TYPE ... ADD VALUE`` va en una conexion AUTOCOMMIT (no puede correr
      dentro de una transaccion) y primero, para que los valores nuevos existan
      antes de cualquier DDL que los referencie.
    - El resto se aplica una por una, con commit por sentencia y rollback +
      aviso si falla, en conexiones propias del engine (NO la transaccion que
      Alembic administra para esta migracion), de modo que un fallo idempotente
      no aborte el resto.
    """
    bind = op.get_bind()
    engine = bind.engine

    add_value_stmts = [s for s in stmts if "ADD VALUE" in s.upper()]
    regular_stmts = [s for s in stmts if "ADD VALUE" not in s.upper()]

    for stmt in add_value_stmts:
        try:
            with engine.connect().execution_options(isolation_level="AUTOCOMMIT") as conn:
                conn.execute(text(stmt))
        except Exception as e:  # noqa: BLE001 — idempotente/tolerante a propósito
...
def upgrade() -> None:
    _run(_PENDING_DDLS)
    _run(_ADD_COLUMN_DDLS)
    _run(_ADD_COLUMN_ENUMS)


def downgrade() -> None:
    # No-op deliberado. Esta es una migracion "baseline" que consolida DDL
    # idempotente historico (ADD COLUMN/CREATE IF NOT EXISTS) mezclado con
    # backfills de datos irreversibles (UPDATE ...). Muchas de estas columnas,
    # indices y tablas son compartidos con el esquema base (create_all) y con
    # migraciones previas, por lo que revertirlas destruiria datos de produccion
    # y romperia el esquema. Igual que en un squash/baseline de Alembic, el
    # downgrade se deja como no-op intencional.
    pass

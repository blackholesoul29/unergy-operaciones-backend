...
def diagnostico_migracion(db: Session = Depends(get_db), _=Depends(get_current_user)):
    ...
    seed O&M: ignora el código MGS y compara tokens), y reporta cuántos ya tienen
    contrato de arriendo, cuántos hay que crear y cuántos quedan sin proyecto. No
    modifica nada."""
    ...
    return {
        ...
        "con_contrato_arriendo":   con_contrato,        # ya tienen contrato de arriendo
        "sin_contrato_arriendo":   len(sin_contrato),   # cruzan con proyecto, falta crear contrato
        "sin_match_de_proyecto":   len(sin_match),      # no cruzan con ningún proyecto (revisar a mano)

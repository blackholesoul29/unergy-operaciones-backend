...
def exposure_current(
    fecha: date | None = Query(None, description="Día a evaluar; por defecto el último día con exposición COMPLETA (precio+generación)"),
    planta_id: int | None = Query(None, description="Filtra por planta. OJO: en contratos multi-planta la obligación se atribuye completa a la planta (sobreestima); ver /exposure/historical."),
    db: Session = Depends(get_db),
    _=Depends(get_current_user),
):
    """Exposición del portafolio (o planta) al mercado de bolsa.

    Metodología (granularidad DIARIA): precio = promedio simple de las 24 horas
    del día; generación = suma real del día; obligación PPA = mínimo mensual del
    contrato prorrateado linealmente entre los días del mes. Un día sin
    generación real cargada sale INCOMPLETO (datos_completos=False), NO con
    generación=0 — para no fabricar déficits ficticios.
    """
...
def exposure_historical(
    start_dt: date = Query(...),
    end_dt: date = Query(...),
    planta_id: int | None = Query(None, description="Filtra por planta. En contratos multi-planta la obligación se atribuye completa a la planta (cota, sobreestima)."),
    db: Session = Depends(get_db),
    _=Depends(get_current_user),
):
    """Serie diaria de exposición + indicadores de riesgo en el rango.

    Los indicadores (VaR 95%, media, min/max, std) se calculan SÓLO sobre los
    días completos (con precio y generación real); los días incompletos aparecen
    en `puntos` con datos_completos=False pero no contaminan las estadísticas.
    """
    if end_dt < start_dt:

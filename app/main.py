...
# NOTE: schema DDLs that used to run on every boot now live in the Alembic
# migration alembic/versions/031_baseline_all_ddls.py. Run `alembic upgrade head`
# to apply the schema; no DDL is executed at application startup anymore.
...
def _run_mandatos_seed() -> None:
    """Seed the mandatos maestra. Schema is managed by Alembic (no DDL here)."""
    try:
...
def _deferred_init():
    ...
    for label, fn in [
        ("mandatos_seed", _run_mandatos_seed),

...
def _nombre_frontera(db, ids: list) -> dict:
    ...
    salida: dict[int, str] = {}
    filas = db.query(Frontera).filter(Frontera.proyecto_id.in_(ids)).all()
...
def calcular_balance(db, year: int, month: int,
    ...
    from app.api.v1.cumplimiento import (
        _fecha_corte, _fetch_month, _fetch_range, _mon_id, _unergy_token,
        get_plantas_contratos,
    )

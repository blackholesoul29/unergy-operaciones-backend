...
real de energía. Se obtiene consultando la API de generación de Unergy
...
COALESCE ``sub_project → alias_monitoreo → topic_slug``: para las plantas que ya
tienen ``sub_project`` no cambia nada, y para las que solo tienen ``topic_slug``
...
def identificador_monitoreo(proyecto) -> str | None:
    ...
    ``sub_project`` es el canónico; si falta, cae a ``alias_monitoreo`` y luego a
    ``topic_slug``. Solo AGREGA cobertura: para plantas con ``sub_project`` el
    resultado es idéntico al de antes.
    """
    return (
        getattr(proyecto, "sub_project", None)
        or getattr(proyecto, "alias_monitoreo", None)
...
def backfill_comercializacion(db, *, force: bool = False, dry_run: bool = False) -> dict:
    """Rellena ``fecha_inicio_comercializacion`` para los proyectos que la
    necesitan, derivándola de la generación real.

    - Sin ``force``: solo toca proyectos con la fecha en NULL y NO editados a mano
      (``fecha_comercializacion_editada_manual = False``). Idempotente: correrlo
    ...
    Devuelve un reporte con las listas para el diagnóstico final.
    """
    ...
    if not force:
        q = q.filter(
            Proyecto.fecha_inicio_comercializacion.is_(None),
            Proyecto.fecha_comercializacion_editada_manual.is_(False),
        )
    else:
        q = q.filter(Proyecto.fecha_comercializacion_editada_manual.is_(False))
    ...
    for p in proyectos:
        ...
        actualizados.append({
            ...
            "anterior": p.fecha_inicio_comercializacion.isoformat() if p.fecha_inicio_comercializacion else None,
        })
        if not dry_run:
...
def proyectos_sin_fecha_comercializacion(db) -> list[dict]:
    """Diagnóstico: proyectos en operación sin fecha de inicio de comercialización.

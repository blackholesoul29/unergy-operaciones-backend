...
# Ventana hacia atrás para "exposición actual" sin fecha: cuántos días retroceder
# buscando el último día con generación real cargada (la generación llega con
# rezago respecto al precio de bolsa).
CURRENT_LOOKBACK_DAYS = 45
...
def get_ppa_obligacion_mwh_por_dia(
    ...
    LIMITACIÓN CONOCIDA (per-planta ≈ nivel-contrato): un contrato puede cubrir
    varias plantas (M:N vía `ppa_contrato_proyectos`), pero `energia_minima` es
    el mínimo del CONTRATO, no por planta. Al filtrar por `planta_id` se
    atribuye el mínimo COMPLETO del contrato a esa planta (no se divide entre
    las plantas inscritas), por lo que la obligación —y la exposición— per-planta
    de un contrato multi-planta queda SOBREESTIMADA. El esquema actual no tiene
    un desglose por planta; la exposición per-planta debe leerse como una cota
    a nivel de contrato. Sin `planta_id` (todo el portafolio) el total es exacto.
    """
...
def get_historical_exposure(
    ...
    for dia in dias:
        ...
        # HONESTIDAD DE DATOS: la generación NO se defaultea a 0. La generación
        # es `fuente="manual"` con rezago de días, mientras precio (lote) y
        # obligación (mensual prorrateada) casi siempre están presentes. Un
        # default 0.0 fabricaría un déficit `(0 − obligación) × precio` enorme
        # en los días recientes y envenenaría VaR/media/min. Sin generación el
        # día queda INCOMPLETO (exposición None → fuera de los indicadores).
        gen = generaciones.get(dia)
        # La obligación SÍ defaultea a 0: su ausencia significa "el contrato no
        # exige mínimo ese mes" (dato de contrato, estable, no rezagado), no un
        # dato faltante.
        obl = obligaciones.get(dia, 0.0)
        exposicion = compute_exposure(gen, obl, precio)
        completo = precio is not None and gen is not None
        puntos.append(
            {
                "fecha": dia,
                "planta_id": planta_id,
...
def calculate_current_exposure(
    ...
    Con `fecha` explícita: la exposición de ese día (puede salir incompleta si
    aún no hay generación cargada, pero NUNCA fabricada con generación=0).

    Sin `fecha`: el ÚLTIMO día con exposición COMPLETA (precio + generación
    real) dentro de los últimos `CURRENT_LOOKBACK_DAYS` — así "exposición
    actual" refleja el dato real más reciente y no el borde donde la generación
    todavía no llegó. Si ningún día del rango tiene generación, devuelve la
    última fecha con precio como punto INCOMPLETO (datos_completos=False,
    exposicion=None), en vez de un déficit ficticio.
    """
    if fecha is not None:
        resultado = get_historical_exposure(db, fecha, fecha, planta_id)
        return resultado["puntos"][0] if resultado["puntos"] else _punto_vacio(fecha, planta_id)

    ultimo = db.query(func.max(PrecioBolsa.fecha_hora)).scalar()
    if ultimo is None:
        return _punto_vacio(None, planta_id)
    last_price_date = _as_date(ultimo)
    inicio = last_price_date - timedelta(days=CURRENT_LOOKBACK_DAYS)
    hist = get_historical_exposure(db, inicio, last_price_date, planta_id)
    completos = [p for p in hist["puntos"] if p["datos_completos"]]
    if completos:
        return completos[-1]
    return _punto_vacio(last_price_date, planta_id)
...
def _punto_vacio(fecha, planta_id) -> dict:
    """Punto de exposición sin datos (honesto, no fabrica ceros)."""
    return {
        "fecha": fecha,
        "planta_id": planta_id,
        "generacion_mwh": None,

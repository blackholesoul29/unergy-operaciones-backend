...
# Rango representable por la columna precio_cop_mwh NUMERIC(10,2). Un precio
# fuera de rango (típicamente por pasar convertir_kwh_a_mwh=True sobre un archivo
# que YA viene en COP/MWh) reventaría el INSERT con un 500 de la BD; se omite la
# fila con un warning en vez de propagar el error.
_PRECIO_MAX = Decimal("99999999.99")
    ...
    *,
    ...
    for _, row in df.iterrows():
        ...
        for col, hora in columnas_hora:
            ...
            if abs(precio) > _PRECIO_MAX:
                logger.warning(
                    "Precio fuera de rango NUMERIC(10,2) (%s COP/MWh @ %s h%02d), fila "
                    "omitida — ¿archivo ya en COP/MWh con convertir_kwh_a_mwh=True?",
                    precio, dia, hora,
                )
                continue
            filas.append(

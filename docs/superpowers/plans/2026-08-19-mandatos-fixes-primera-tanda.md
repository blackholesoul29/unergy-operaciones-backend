...
**Goal:** Corregir los tres defectos que destapó la primera corrida contra 94 correos reales, y que ningún fixture iba a mostrar.
...
Queda un cuarto defecto fuera de este plan: **cero correos clasificados `molde_simple`**, o sea que `extraer_observaciones` nunca corre y la Fuente 1 está muerta. Los asuntos reales son `Re: Revisión mandatos de costos - Julio`; el `Re:` los manda a `seguimiento` y el resto no matchea las frases del molde. Arreglarlo exige ver el cuerpo de un correo real de correcciones — sin eso se volverían a inventar frases, que es como se llegó acá.
...
- **La Fuente 1 sigue muerta** (cero `molde_simple`). Necesita el cuerpo de un

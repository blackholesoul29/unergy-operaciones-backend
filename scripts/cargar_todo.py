"""
Script de carga masiva: clientes pendientes + proyectos + inversionistas
Corre cuando el backend de Railway esté disponible.
"""
import requests
...
# ── 2. PROYECTOS ──────────────────────────────────────────────────────────────
# cliente_id se asigna dinamicamente despues de cargar clientes
...
# porcentaje en decimal (ej: 77.19% -> 0.7719)
...
def main():
    ...
    for c in CLIENTES_PENDIENTES:
        r = requests.post(f"{BASE}/api/v1/clientes", json=c, headers=h, timeout=30)
        if r.status_code == 401:
            h = get_token()
            r = requests.post(f"{BASE}/api/v1/clientes", json=c, headers=h, timeout=30)
        if r.status_code in (200, 201):
            ok += 1; print(f"  OK {c['razon_social_nombre']}")
        elif r.status_code == 422 and "unique" in r.text.lower():
            print(f"  YA EXISTE {c['razon_social_nombre']}")
        else:
            err += 1; print(f"  ERROR {c['razon_social_nombre']} - {r.status_code}: {r.text[:100]}")
    print(f"Clientes: {ok} nuevos, {err} errores")

    # 2. Obtener mapa nombre->id de todos los clientes
    print("\nObteniendo mapa de clientes...")
    r = requests.get(f"{BASE}/api/v1/clientes?size=200", headers=h, timeout=30)
    clientes_map = {c["razon_social_nombre"].strip(): c["id"] for c in r.json()["items"]}
    print(f"  {len(clientes_map)} clientes en BD")
    ...
    for p in PROYECTOS_RAW:
        ...
        r = requests.post(f"{BASE}/api/v1/proyectos", json=payload, headers=h, timeout=30)
        if r.status_code == 401:
            h = get_token()
            r = requests.post(f"{BASE}/api/v1/proyectos", json=payload, headers=h, timeout=30)
        if r.status_code in (200, 201):
            ok += 1
            proyectos_map[p["nombre_comercial"]] = r.json()["id"]
            print(f"  OK {p['nombre_comercial']}")
        else:
            err += 1; print(f"  ERROR {p['nombre_comercial']} - {r.status_code}: {r.text[:100]}")
    print(f"Proyectos: {ok} creados, {err} errores")
    ...
    for inv in INVERSIONISTAS:
        ...
        if not proy_id or not cli_id:
            print(f"  SKIP {inv['proyecto']} / {inv['cliente']} (no encontrado)")
            continue
        payload = {"cliente_id": cli_id, "porcentaje_participacion": inv["pct"]}
        r = requests.post(f"{BASE}/api/v1/proyectos/{proy_id}/inversionistas", json=payload, headers=h, timeout=30)
        if r.status_code == 401:
            h = get_token()
            r = requests.post(f"{BASE}/api/v1/proyectos/{proy_id}/inversionistas", json=payload, headers=h, timeout=30)
        if r.status_code in (200, 201):
            ...
            err += 1; print(f"  ERROR {inv['proyecto']} / {inv['cliente']} - {r.status_code}: {r.text[:100]}")
    print(f"Inversionistas: {ok} creados, {err} errores")

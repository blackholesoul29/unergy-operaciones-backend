...
def test_editar_permite_guardar_el_mismo_codigo_frontera_sin_cambios(db):
    ...
    out = api.update_frontera(f.id, FronteraUpdate(codigo_frontera="FRT00001", agente_exportador="Corozal"),
                               forzar=False, db=db, _=ADMIN)
    assert out.agente_exportador == "Corozal"
...
def test_editar_no_se_compara_consigo_misma(db):
    ...
    out = api.update_frontera(f.id, FronteraUpdate(nombre_frontera="La Catedral", agente_exportador="Corozal"),
                               forzar=False, db=db, _=ADMIN)
    assert out.agente_exportador == "Corozal"

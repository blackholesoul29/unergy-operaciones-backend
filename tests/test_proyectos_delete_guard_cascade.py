...
@pytest.mark.parametrize("tabla,fila", [
    ...
    # A diferencia de las 6 de arriba, esta NO es ON DELETE CASCADE (es NO
    # ACTION) -- el riesgo no es perder el dato en silencio, sino un
...
def test_tabla_sin_relationship_en_el_modelo_bloquea_el_borrado(db, tabla, fila):
    ...
    Postgres las borraba en cascada sin aviso; para la última
    (proyecto_informe_om, ON DELETE NO ACTION), el borrado reventaba con un

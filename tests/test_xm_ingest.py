...
# ── normalización de unidad (kWh/MWh/GWh → MWh) ──────────────────────────────
from app.services.xm_ingest_service import _detect_unit  # noqa: E402


def test_detect_unit_recognizes_energy_units():
    assert _detect_unit("Generación (MWh)") == "mwh"
    assert _detect_unit("Generación kWh") == "kwh"
    assert _detect_unit("Energía GWh") == "gwh"
    # Sin unidad reconocible → None (no se adivina).
    assert _detect_unit("Generación") is None
    assert _detect_unit("Energía") is None


def test_parse_rows_converts_kwh_to_mwh():
    """Un archivo en kWh debe normalizarse a MWh (÷1000) — evita el error 1000×."""
    lookup = {"el copey": 7}
    rows = [{"proyecto": "El Copey", "meter_id": "M1",
             "fecha": "01/06/2026", "generacion": 12500}]
    valid, errors = _svc().parse_rows(rows, lookup, unit_factor=Decimal("0.001"))
    assert not errors
    assert valid[0]["generation_mwh"] == Decimal("12.500")


def test_parse_rows_converts_gwh_to_mwh():
    lookup = {"el copey": 7}
    rows = [{"proyecto": "El Copey", "meter_id": "M1",
             "fecha": "01/06/2026", "generacion": 2}]
    valid, _ = _svc().parse_rows(rows, lookup, unit_factor=Decimal("1000"))
    assert valid[0]["generation_mwh"] == Decimal("2000")


def test_read_rows_rejects_generation_column_without_unit():
    """Sin unidad en el encabezado de generación, falla fuerte (no adivina MWh)."""
    buf = _xlsx(
        ["Proyecto", "Medidor", "Fecha", "Generación"],
        [["El Copey", "FRT123", "01/06/2026", 12.5]],
    )
    svc = XMIngestionService(db=None)
    try:
        svc._read_rows(buf)
        assert False, "debió lanzar ValueError por unidad indeterminable"
    except ValueError as exc:
        assert "unidad" in str(exc).lower()


def test_read_rows_accepts_kwh_header():
    buf = _xlsx(
        ["Proyecto", "Medidor", "Fecha", "Generación kWh"],
        [["El Copey", "FRT123", "01/06/2026", 12500]],
    )
    rows, col_map = XMIngestionService(db=None)._read_rows(buf)
    assert _detect_unit(col_map["generacion"]) == "kwh"
    assert rows[0]["generacion"] == 12500
